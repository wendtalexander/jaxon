...
fig.add_scatter(x=voltages, y=theory, mode="lines")
```
...
fig.add_scatter(x=f, y=pxx_th, name="Theory")
...
f, pxx_segmemts = jsp.signal.welch(v_segments, fs=fs, nperseg=nperseg)
...
fig = go.Figure()
fig.add_scatter(x=f, y=pxx_rfft, name="welch")
fig.add_scatter(x=f, y=pxx_th, name="Theory")
fig.update_yaxes(type="log")
fig.update_xaxes(range=[0, 15])
```
...
```{python}
tau = nperseg * fs
fft_pxx = jnp.fft.fft(v_segments)
pxx_fft = (jnp.abs(fft_pxx) ** 2) / tau
pxx_fft = pxx_fft.sum(axis=0) / num_segments


fig = go.Figure()
fig.add_scatter(x=f, y=pxx_fft, name="welch")
fig.add_scatter(x=f, y=pxx_th, name="Theory")
fig.update_yaxes(type="log")
fig.update_xaxes(range=[0, 15])
```
...
fig.add_scatter(x=f, y=pxx_fft, name="welch")
fig.add_scatter(x=f, y=pxx_fft, name="fft")
